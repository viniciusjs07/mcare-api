...
exports.employee_get_all = async (req, res, callback) => {
    const employers = await Employee.find({}, (err) => {
        if(err){
            console.error(err);
            return res.status(400).send({error: err.message});
        }
    }).select('_id name role gender phone');

    return res.status(200).send({employers});
};

exports.employee_create = (req, res, callback) => {
...
exports.employee_detail = (req, res, callback) => {
    Employee.findById(req.params.id, (err, employee) => {
        if(err){
            ...
            if (employee === null || employee === undefined) res.status(404).send({message: 'Usuário não encontrado'});
            else res.status(200).send({employee});
        }
    });
};

exports.employee_update = (req, res, callback) => {
...
exports.employee_delete = (req, res, callback) => {
